...
//		"io"
//		"net/http"
//		"github.com/h4ckm03d/simpleplan/pat"
//		"log"
//	)
//
...
func (p *PatternServeMux) Head(pat string, h http.HandlerFunc) {
	p.Add("HEAD", pat, h)
}
...
func (p *PatternServeMux) Get(pat string, h http.HandlerFunc) {
	p.Add("HEAD", pat, h)
	p.Add("GET", pat, h)
}

// Post will register a pattern with a handler for POST requests.
func (p *PatternServeMux) Post(pat string, h http.HandlerFunc) {
	p.Add("POST", pat, h)
}

// Put will register a pattern with a handler for PUT requests.
func (p *PatternServeMux) Put(pat string, h http.HandlerFunc) {
	p.Add("PUT", pat, h)
}

// Del will register a pattern with a handler for DELETE requests.
func (p *PatternServeMux) Del(pat string, h http.HandlerFunc) {
	p.Add("DELETE", pat, h)
}

// Options will register a pattern with a handler for OPTIONS requests.
func (p *PatternServeMux) Options(pat string, h http.HandlerFunc) {
	p.Add("OPTIONS", pat, h)
}

// Patch will register a pattern with a handler for PATCH requests.
func (p *PatternServeMux) Patch(pat string, h http.HandlerFunc) {
	p.Add("PATCH", pat, h)
}

// Add will register a pattern with a handler for meth requests.
func (p *PatternServeMux) Add(meth, pat string, h http.HandlerFunc) {
	p.add(meth, pat, h, false)
}

func (p *PatternServeMux) add(meth, pat string, h http.HandlerFunc, redirect bool) {
